...
class StudentCredentialsCardsPdfGeneratorImpl : StudentCredentialsCardsPdfGenerator {
    ...
    override fun generate(studentClassroomCards: List<StudentClassroomCard>): InputStream {
        ...
        val totalCards = studentClassroomCards.size
        ...
        for (pageIdx in 0 until pageCount) {
            val cards = studentClassroomCards
    ...
    private fun addCardsPage(
        document: PDDocument,
        studentClassroomCards: List<StudentClassroomCard>,
        textFont: PDFont,
        ...
        studentClassroomCards.forEachIndexed { cardIdx, card ->
    ...
    private fun addCard(
        ...
        drawCredentials(stream, textFont, cardCenterX, rectangle, card)
    ...
    private fun drawCredentials(
        ...
        stream.showText("Username: ${card.username}")
        stream.newLine()
        stream.showText("Class access code: ${card.classroomAccessCode}")
        stream.newLine()
        stream.showText("Password: ${card.password}")
        stream.endText()
    }
